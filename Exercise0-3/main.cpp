...
int main()
{
    ...
    // Matrix:
    Matrix2d Ad = Matrix2d::Identity(); // fixed dimenional matrix 2x2 of double : intiailized as the identity matrix
    Matrix2i Ai = Matrix2i::Zero(); // fixed dimensions available: 2x2 3x3 4x4


    std::cout << Ad << std::endl;

    MatrixXd B = MatrixXd::Ones(3, 3); // variable dimensional matrix: this is initialized as an all-ones matrix 2x2

    cout << "B matrix:" << endl;
    cout << B << endl;

    B.resize(4, 4); // a dynamic matrix can be resized -> resize is not conservative! Try the conservativeResize() method.
    // B.resize(2, 2);
    ...
    srand(4); // set seed for random matrix initialization
